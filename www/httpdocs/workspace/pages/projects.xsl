...
<xsl:template match="data">
	...
	<div class="projects clear">
		...
		<xsl:apply-templates select="//projects-all-entries-by-date/entry" />
...
<xsl:template match="//projects-all-entries-by-date/entry">

	<div class="entry">
		<a href="{url}" target="_blank" class="cover">
			<xsl:attribute name="style">background-image: url('<xsl:value-of select="$root" />/image/2/300/300/2/0<xsl:value-of select="cover/@path" />/<xsl:value-of select="cover/filename" />')</xsl:attribute>
		</a>
		<h5><a href="{url}" target="_blank"><xsl:value-of select="name" disable-output-escaping="yes" /></a></h5>
	</div>
